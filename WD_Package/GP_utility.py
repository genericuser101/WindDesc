#This script has all the functionality of the Gaussian Process and the turbines.
#Our standard set of imports
import pandas as pd
import numpy as np
...
class Turbine_Helper():
    def __init__(self):
        ...
        self.Gs = {self.symbol: Turbine_Helper.load_turbine_parameters(self.symbol)}
...
class GP_Helper():
    ...
    def train_model(self, filename):
        # LOAD DATA
        dataset_full = pd.read_csv(filename, index_col=0)
        symbol="Siemens"
        k=0
        count=0
    ...
    def predict_model(self, trained_GP_model, pos_array, num_turbs_predict):
        ...
        position = pos_array
            # Calculate fingerprint
        fingerprints =  self.turbine_utility.fingerprint(position, num_turbs_predict)
            # Predict wind speed
        refwind, refstdev = trained_GP_model.predict(fingerprints)
            
        return refwind, refstdev
